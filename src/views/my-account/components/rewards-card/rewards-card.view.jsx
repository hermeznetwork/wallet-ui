        ...
        return isRewardActive === false ? (
          <>
            ...
            {accountEligibilityTask.data ? (
              <p className={classes.rewardText}>
                Your total reward is {getFormattedEarnedReward(earnedRewardTask.data)} HEZ (
          ...
          <>
            <p className={classes.rewardText}>
              Reward over your funds is{" "}
              <span className={classes.rewardPercentage}>
                {getFormattedPercentage(rewardPercentageTask.data) || "--"}%
              </span>
              .&nbsp;
              {accountEligibilityTask.data ? (
                <span>
                  You earned so far {getFormattedEarnedReward(earnedRewardTask.data)} HEZ (
                  {CurrencySymbol[preferredCurrency].symbol}
                  {getRewardAmountInPreferredCurrency(earnedRewardTask.data)}).
                </span>
              ) : (
                <span>
                  You earned so far 0.00 HEZ ({CurrencySymbol[preferredCurrency].symbol}0.00).
                </span>
              )}
            </p>
          </>
